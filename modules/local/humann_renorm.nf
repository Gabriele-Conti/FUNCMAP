process HUMANN_RENORM {
  ...
  // Publish all normalized/non-normalized HUMAnN tables to the final HUMANN_OUTS directory.
  publishDir "${params.outdir}/HUMANN_OUTS", mode: 'copy'

  input:
  // Inputs produced by HUMANN_RENAME / HUMANN_REGROUP.
  // Both original and named tables are processed when available.
  tuple val(sample_id),
        path(gene_fam),
        path(path_abund),
        path(ko_tsv),
  ...
  output:
  // Emit all per-sample HUMAnN TSV tables generated by this step.
  tuple val(sample_id),
        path("${sample_id}/*.tsv")
